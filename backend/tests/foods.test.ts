...
describe('GET /api/foods/search (legacy contract)', () => {
  ...
  it('returns [] when FatSecret has no matches', async () => {
    ...
    const res = await request(app).get('/api/foods/search').query({ query: 'zzz' });
  ...
  it('rate limits per IP with the legacy body and a Retry-After header', async () => {
    ...
    await request(app).get('/api/foods/search').query({ query: 'a' });
    await request(app).get('/api/foods/search').query({ query: 'a' });
    const res = await request(app).get('/api/foods/search').query({ query: 'a' });
