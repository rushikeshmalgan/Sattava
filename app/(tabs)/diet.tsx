...
export default function DietScreen() {
  ...
  // Loading & error states for the plan tab
  ...
  const loadPlan = async () => {
    ...
    try {
      ...
      setPlanError(err?.message ?? 'Could not load your meal plan. Tap to retry.');
    } finally {
        ...
        >
          {isPlanLoading ? (
            <View style={styles.emptyState}>
              ...
              <Text style={[styles.emptySubtitle, { marginTop: 16 }]}>Loading your meal plan…</Text>
            </View>
          ) : planError ? (
            <View style={styles.emptyState}>
